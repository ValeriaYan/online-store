...
class ProductInCart extends ComponentTemplate {
  ...
  private createProductInfo() {
    const pictureWrap = document.createElement('a');
    pictureWrap.className = 'item__link';
    pictureWrap.href = `/product/${this._cartProduct.product.id}`;
  ...
  private createControls(reduceAmountProduct: Function) {
    ...
    buttonSub.addEventListener(
      'click',
      this.changeQuantityHandler.bind(this, 'sub', reduceAmountProduct)
    );
    buttonPlus.addEventListener(
      'click',
      this.changeQuantityHandler.bind(this, 'plus', reduceAmountProduct)
    );
